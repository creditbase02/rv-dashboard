...
function validateSupplyTop(value, name, denominator) {
  if (!Array.isArray(value) || value.length > 5) throw new Error(`Supply ${name} Top 5 結構無效`);
  const total = validateSupplyPairs(value, name, true);
  if (value.some(row => row[1] <= 0) || total > denominator) throw new Error(`Supply ${name} Top 5 無效`);
...
export function validateSupplySnapshot(data) {
  ...
  for (const definition of data.peer_definitions) {
    if (!sameKeys(definition, ['name','tickers']) || typeof definition.name !== 'string' || !definition.name || ['Others',OTHER_IG].includes(definition.name) || peerNames.includes(definition.name) || !Array.isArray(definition.tickers) || !definition.tickers.length) throw new Error('Supply Peer Group 定義無效');
  ...
  const allPeers = [...peerNames, OTHER_IG];
  ...
  if (!sameKeys(top, ['ytd','monthly']) || !sameKeys(top.ytd, ['industry','rating','peer_group'])) throw new Error('Supply Top 5 結構不正確');
  for (const field of ['industry','rating','peer_group']) {
    const expected = data.breakdowns[field], rows = top.ytd[field];
    if (!Array.isArray(rows) || rows.map(row => row?.[0]).join('|') !== expected.map(row => row[0]).join('|')) throw new Error(`Supply ${field} Top 5 分類不正確`);
    const totals = Object.fromEntries(expected); for (const [name,tickers] of rows) validateSupplyTop(tickers, `${field} ${name}`, totals[name]);
  }
  for (const name of peerNames) if (JSON.stringify(Object.fromEntries(top.ytd.peer_group)[name]) !== JSON.stringify(data.peer_tickers[name].slice(0,5))) throw new Error(`Supply ${name} Top 5 無法勾稽`);
  if (!sameKeys(top.monthly, ['total','peer_groups']) || !Array.isArray(top.monthly.total) || top.monthly.total.length !== 12) throw new Error('Supply 月度 Top 5 結構不正確');
  top.monthly.total.forEach((rows,month) => validateSupplyTop(rows, `${month + 1} 月`, data.monthly.total[month]));
  if (!Array.isArray(top.monthly.peer_groups) || top.monthly.peer_groups.map(row => row?.[0]).join('|') !== allPeers.join('|')) throw new Error('Supply 月度 Peer Top 5 分類不正確');
  const monthlyPeer = Object.fromEntries(data.monthly.peer_groups); for (const [name,months] of top.monthly.peer_groups) { if (!Array.isArray(months) || months.length !== 12) throw new Error(`Supply ${name} 月度 Top 5 結構不正確`); months.forEach((rows,month) => validateSupplyTop(rows, `${month + 1} 月 ${name}`, monthlyPeer[name][month])); }
